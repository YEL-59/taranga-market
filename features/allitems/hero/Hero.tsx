                ...
                >
                  {isSearching ? (
                    ...
                    <ReactLenis
                      ...
                      className="max-h-[400px] overflow-y-auto w-full p-2"
                    >
                      <div className="px-4 py-3 border-b border-gray-50 flex items-center justify-between">
                        <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">
                          Results
                        </span>
                        <span className="text-xs font-bold text-[#227c85] bg-[#227c85]/10 px-2 py-1 rounded-md">
                      ...
                      {searchResults.map((item) => (
                        <div
                          key={item.id}
                          onClick={() =>
                            router.push(`/featured-details?id=${item.id}`)
                          }
                        ...
                        >
                          ...
                          <div className="flex flex-col min-w-0 flex-1">
                            <h4 className="text-sm font-bold text-gray-800 truncate group-hover:text-[#227c85] transition-colors">
                              {item.title}
                            </h4>
                            <span className="text-xs text-gray-500 truncate mt-1">
                              {item.category?.name || "Product"} • {item.city} ,{" "}
                              {item.state}
                            </span>
                          </div>
                          <div className="text-right shrink-0">
                            <span className="text-sm font-extrabold text-[#ff6b3d]">
                              {item.price ? `${item.price} CFA` : "N/A"}
                            </span>
                          </div>
                        </div>
                      ))}
                      ...
                      >
                        View all results{" "}
                        <LinkIcon className="w-3.5 h-3.5 inline ml-1" />
                      </div>
                    </ReactLenis>
                  ) : (
