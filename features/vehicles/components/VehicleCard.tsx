...
const VehicleCard: React.FC<VehicleCardProps> = ({ item, onClick }) => {
    ...
    const metaStr = item.meta || { year: '2024', mileage: '0 km' };
    ...
    return (
        ...
        >
            <div className="relative aspect-[16/10] w-full overflow-hidden bg-gray-100 rounded-[15px]">
                {image && <Image src={image} alt={item.title || "Vehicle"} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />}
            ...
            <CardContent className="flex flex-col flex-1 p-3.5 pt-4">
                ...
                <div className="mb-4 flex items-center gap-4 text-[12px] text-gray-500">
                    <div className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5 text-gray-400" /><span>{metaStr.year}</span></div>
                    <div className="flex items-center gap-1.5"><Gauge className="h-3.5 w-3.5 text-gray-400" /><span>{metaStr.mileage}</span></div>
                </div>
